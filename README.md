...
- The second argument indicates the model chosen for the flux emitted by the neutrino sources (either `SPL`, `SPLcut`, or `BPL`)
...
**Before running fits using `SPLcut` or `BPL` emission models**, you will need to pre-compute a series of splines. This can be done by calling `julia scripts/save_calcs_for_zdist.jl $ZNUM`, where `ZNUM` is an integer indicating the redshift evolution function chosen. The results will be saved to a folder `saved/`. 
